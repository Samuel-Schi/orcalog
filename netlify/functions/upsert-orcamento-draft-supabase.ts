...
export const handler: Handler = async (event) => {
  try {
    ...
    const tableName = sanitizeIdentifier(
      process.env.SUPABASE_ORCAMENTOS_DRAFT_TABLE || 'orcamento_rascunhos',
      'orcamento_rascunhos'
    );
    ...
    const restUrl = `${normalizedSupabaseUrl}/rest/v1/${tableName}?on_conflict=${encodeURIComponent('pa_usuario')}`;
    ...
    if (!response.ok) {
      return jsonResponse(response.status, {
        error: 'Falha ao salvar rascunho no Supabase.',
        detail: text || null
      });
    }
